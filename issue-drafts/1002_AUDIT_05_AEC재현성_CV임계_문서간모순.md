...
## 종결 (2026-09-17, 드레인 스프린트 — plan #1048 기본값(b) 자동 적용)
- plan #1048 기한 **2026-09-16 경과, (a) 원문 구매 승인 미상신** → 사전 지정된 **기본값 (b) 보수적 잠정값 채택**을 자동 적용.
- 적용 내용: AEC 재현성 CV 역치를 **≤5%(잠정)** 로 4개 문서 7개소 일원화 — 검사_시험_밸리데이션_개요 L110·L195, SOP-IQ-001 L199·L351, SOP-CAL-001 L180, 영상품질_QC_프로토콜 L133·L237. 각 문서 말미 **주1**에 "IEC 60601-2-54:2022 Table 203.102 원문 미확인 · 잠정값 · 원문 확보 시 재검증" 명기.
- 보수적 값 선정 근거: 두 후보(5%/10%) 중 **엄격한 쪽**을 채택(안전측 판단). 규범 판정값 주장 아님 — 추정 배제 원칙은 "정답 확정" 미주장으로 유지.
- 조항 귀속(§203.6 / Table 203.102)은 7차 정정분 그대로 유지. **문서 간 모순 0.**
- 재검증 트리거: IEC 60601-2-54:2022 원문 확보 시 Table 203.101/203.102 대조 후 주1 해제·값 확정.
- **state: closed.** 실운영 문서 미참고. web_verification: no(방침 적용 사이클).
