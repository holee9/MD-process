---
doc-id: T-FDA510K-B2_Intended_Use_템플릿
title: FDA 510(k) Intended Use / Indications for Use 템플릿
type: Form
version: v0.1
status: draft
category: 01_법규_규제
purpose: FDA 510(k) RTA Checklist B.2 (Intended Use/IFU) 및 FDA Form 3881 대응 템플릿 — 골격만 정의, 제품별 실 데이터 작성 예정
applicable: [FDA 510(k), FDA QMSR]
owner: RA 담당자
last-review: 2026-09-10
review-due: 2027-09-10
---

# T-FDA510K-B2 FDA 510(k) Intended Use / Indications for Use 템플릿 (v0.1 — 골격)

> **주의**: 본 문서는 FDA_510k_RTA.md 항목 FDA-510K-B2(및 Form 3881) 대응 골격 템플릿이다. 제품 특정 데이터는 **작성 예정**이며, 임의의 데이터를 채우지 말 것.

## 개정 이력

| 버전 | 일자 | 변경 내용 | 작성자 |
|------|------|-----------|--------|
| v0.1 | 2026-09-10 | 골격 템플릿 신규 생성 (FDA-510K-B2 증빙 미비 해소) | RA 담당자 |

---

## 구성 항목 (FDA RTA Checklist Section B.2 / Form 3881 기준)

| 항목 | 내용 |
|------|------|
| Intended Use (기기의 일반적 목적) | 작성 예정 |
| Indications for Use (구체적 적응증) | 작성 예정 |
| 대상 질환/상태 | 작성 예정 |
| 대상 환자군(연령·상태 등) | 작성 예정 |
| 사용 환경(병원/가정/이동 등) | 작성 예정 |
| Prescription (Rx) vs OTC 구분 | 작성 예정 |
| 금기사항(Contraindications) | 작성 예정 |

## 관련 문서

| 문서 | 연계 내용 |
|------|-----------|
| 13_규제평가_체크리스트/FDA_510k_RTA.md | FDA-510K-B1~B3 |
| 01_법규_규제/03_미국_FDA/T-FDA510K-B1_Device_Description_템플릿.md | Device Description과 정합 |
| 01_법규_규제/03_미국_FDA/T-FDA510K-A1_Cover_Letter_템플릿.md | Cover Letter의 IFU 요약과 정합 |

## 출처

- FDA, "Refuse to Accept Policy for 510(k)s" Guidance Section B.2
- FDA Form 3881 (Indications for Use) — OMB No. 0910-0120, 최신 개정판 원문 대조 필요 (audit #1028 정정)
