...
def build_maintenance(config):
    ...
    H.append(STYLE)
    H.append('<style>.actions a[data-tip]{position:relative}.actions a[data-tip]:hover::after{content:attr(data-tip);position:absolute;left:0;top:128%;width:290px;white-space:normal;text-align:left;background:#0b1020;color:#e2e8f0;border:1px solid #475569;border-radius:7px;padding:9px 11px;font-size:.72rem;line-height:1.5;font-weight:400;z-index:30;box-shadow:0 8px 22px rgba(0,0,0,.5)}.actions a[data-tip]:hover::before{content:"";position:absolute;left:16px;top:118%;border:6px solid transparent;border-bottom-color:#475569;z-index:30}</style>')
    ...
    H.append(f'<a href="{_wk_btn}" target="_blank" data-tip="매주 1회 점검용. 통과율 추세·열린 audit P0·재감사 통과·해석범위를 확인하고 결정/지시를 기록합니다. 제출하면 review·weekly 라벨 이슈로 추적됩니다.">▶ 주간 점검 기록</a>')
    H.append(f'<a href="{_focus_btn}" target="_blank" data-tip="통과율 하락이나 특정 영역 보강이 필요할 때. 대상 조항·영역과 사유를 적어 제출하면 source:plan·P1 이슈가 되어 빌더가 우선 보강합니다.">▶ 집중 영역 지시</a>')
    H.append(f'<a href="{_ra_btn}" target="_blank" data-tip="법 해석이 갈릴 수 있는(유권해석 범위) 사안. 1차 근거와 갈리는 지점을 적고 RA가 결정합니다. review·interpretation 이슈로 남고, 감사자는 이런 건 사실오류가 아니라 해석범위로 분류합니다.">▶ RA 유권해석 판단</a>')
